...
The first console UI has 3 choices, adding, merging, or deleting all items.
...
After creating the type you want, you must add a new item in the dictionary called TypeCelestialObjectsPair, which takes the type as a key and returns the list it has for that type, so for example we must do this: [typeof(Moon)] = new List<ICelestialObject>(). You can find it in line 30
You also need to add the new type to the dictionary called typeNamePair, you put the type as the key and it returns the string name for your type. You can find it in line 37.
...
Then you must go to line 124, where you can find all the switch cases for the choices. You just need to add a new case, put it in  AssignCelestialObjectInfo method, which takes the TypeCelestialObjectsPair dictionary, you put the key for the type you want in it, you must also put the list of the types this celestial object has, and finally a method that returns a new object of the type you want. You must create the method for your new type. For better readabilty, you can put it next to the CreateNew(YourNewType) methods, you can find them starting from line 259.

In line 145, you can find the merge cases, you just need to edit the console UI and add the new type, and also add a new case in the switch. You must use celestialsMerger object to call CreateAndAddMergedCelestial method, and put TypeCelestialObjectsPair dictionary in the argument, and the method that create your new type.
...
I tried to make all the methods Generic<T>, for better scalability, so you don't need to worry about the other parts. I also made sure that all inputs must be correct, so you can't assign a negative nor a 0 mass, radius, age, and the name also must be unique (case insensitive), and has atleast 1 character. You can find the logic for the name in line 198 and below, and for the other numbers handling in a method called CheckFormat, in line 299.
